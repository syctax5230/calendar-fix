...
// ============================================
// Database Helpers - Customer Profiles
// ============================================
...
async function getCustomerProfile(userId) {
  try {
    const db = getDatabase();
    if (!db) {
      const stored = JSON.parse(localStorage.getItem(`customerProfile_${userId}`) || 'null');
      return stored;
    }

    const profileRef = ref(db, `customerProfiles/${userId}`);
    ...
    console.warn('Error getting customer profile:', error);
    const stored = JSON.parse(localStorage.getItem(`customerProfile_${userId}`) || 'null');
    return stored;
  }
}

async function saveCustomerProfile(userId, profile) {
  try {
    const db = getDatabase();
    if (!db) {
      localStorage.setItem(`customerProfile_${userId}`, JSON.stringify(profile));
    ...
    const profileRef = ref(db, `customerProfiles/${userId}`);
    await set(profileRef, { ...profile, updatedAt: Date.now() });
    localStorage.setItem(`customerProfile_${userId}`, JSON.stringify(profile));
    return profile;
  } catch (error) {
...
// ============================================
// Database Helpers - Customer Warnings
// ============================================
...
async function getCustomerWarningInfo(userId) {
  try {
    const db = getDatabase();
    if (!db) {
      const stored = JSON.parse(localStorage.getItem(`warningInfo_${userId}`) || '{"warnings":0,"isBanned":false}');
      return stored;
    }

    const warningRef = ref(db, `customerWarnings/${userId}`);
    const snapshot = await get(warningRef);
    if (snapshot.exists()) {
      const info = snapshot.val();
      localStorage.setItem(`warningInfo_${userId}`, JSON.stringify(info));
      return info;
    }
    return { warnings: 0, isBanned: false };
  } catch (error) {
    console.warn('Error getting warning info:', error);
    const stored = JSON.parse(localStorage.getItem(`warningInfo_${userId}`) || '{"warnings":0,"isBanned":false}');
    return stored;
  }
}

async function updateCustomerWarningInfo(userId, warningInfo) {
  try {
    const db = getDatabase();
    if (!db) {
      localStorage.setItem(`warningInfo_${userId}`, JSON.stringify(warningInfo));
      return warningInfo;
    }

    const warningRef = ref(db, `customerWarnings/${userId}`);
    await set(warningRef, warningInfo);
    localStorage.setItem(`warningInfo_${userId}`, JSON.stringify(warningInfo));
    return warningInfo;
  } catch (error) {
    console.error('Error updating warning info:', error);
    throw error;
  }
}

// ============================================
// Database Helpers - Booking History Log
// ============================================

async function logBookingHistory(logEntry) {
  try {
    const db = getDatabase();
    const entry = {
      ...logEntry,
    ...
    if (!db) {
      const logs = JSON.parse(localStorage.getItem('bookingHistory') || '[]');
      logs.push(entry);
      localStorage.setItem('bookingHistory', JSON.stringify(logs));
      return entry;
    }

    const historyRef = ref(db, `bookingHistory/${entry.id}`);
    await set(historyRef, entry);
    return entry;
  } catch (error) {
    console.warn('Error logging booking history:', error);
  }
}

// ============================================
// Database Helpers - Update Booking
// ============================================
...
window.updateCustomerWarningInfo = updateCustomerWarningInfo;
window.logBookingHistory = logBookingHistory;
...
export {
  ...
  getCustomerProfile,
  saveCustomerProfile,
  getCustomerWarningInfo,
  updateCustomerWarningInfo,
  logBookingHistory,
  updateBooking,
  getUpliftRequests,
  updateUpliftRequest,
